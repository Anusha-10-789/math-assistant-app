...
export async function requestPasswordReset(username: string, email: string): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(await extractErrorMessage(response, "Failed to send the reset email."));
  }
}
...
export async function generateLesson(
  ...
  const response = await fetch(`${API_BASE_URL}/generate`, {
    ...
    body: JSON.stringify({ topic, grade, num_questions: numQuestions, subject }),
  });
...
export async function fetchQuestionVideo(mcq: MCQItem, grade: number): Promise<Blob> {
  const response = await fetchWithTimeout(
    `${API_BASE_URL}/question-video`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", ...authHeaders() },
      body: JSON.stringify({ mcq, grade }),
    },
    VIDEO_FETCH_TIMEOUT_MS,
  );
  ...
  if (!response.ok) {
    throw new Error(await extractErrorMessage(response, "Failed to generate the explanation video."));
  }
